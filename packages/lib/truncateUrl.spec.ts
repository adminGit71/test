...
describe('truncateUrl', () => {
  ...
  test('should not truncate *.qstnus.com urls', () => {
    const mainnetUrl = 'https://qstnus.com/long/pathname/test';
    const testnetUrl = 'https://testnet.qstnus.com/long/pathname/test';
    expect(truncateUrl(mainnetUrl, 20)).toEqual(
      'qstnus.com/long/pathname/test'
    );
    expect(truncateUrl(testnetUrl, 20)).toEqual(
      'testnet.qstnus.com/long/pathname/test'
    );
  });
});
